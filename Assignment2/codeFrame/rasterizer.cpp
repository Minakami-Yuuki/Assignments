...
{   
    ...
    Vector3f p = {x * 1.0f, y * 1.0f, 1.0f};

    // .z() is mod function.
    // judge the symbols of cross(v0, v1, v2, p)
    // if all symbols are > 0, that means the point is inside
...
void rst::rasterizer::rasterize_triangle(const Triangle& t) {
    ...
    // iterate all bounding box
    for(int ix = minX; ix <= maxX; ix++) {
        for(int iy = minY; iy <= maxY; iy++) {
            int id = 0;
            for(int i = 0; i < N; i++) {
                for(int j = 0; j < N; j++) {
                    //define the mid point d:
                    float d = 1.0 / (2 * N);
                    float x = ix + 2 * d * i + d;
                    float y = iy + 2 * d * j + d;
    ...
    // z_interpolated *= w_reciprocal;
                    if(insideTriangle(x, y, t.v)) {
                        ...
                        // Z-buffer replace the closer point 
                        // and replace the color;
            ...
            for(int i = 0; i < N; i++) {
                for(int j = 0; j < N; j++) {
                    // store the color and relpace the function of getColor().
...
// define depcolor(float first, vector3f second)
// first is Z-buffer's depth
// second is color.
void rst::rasterizer::clear(rst::Buffers buff)
{
    if ((buff & rst::Buffers::Color) == rst::Buffers::Color)
    {
        ...
        for(int i = 0; i < height * width; i++) {
            for(int j = 0; j < N * N; j++) {
                // color: initial the same color.
                depColor[i][j].second = Eigen::Vector3f{0, 0, 0};
            }
        }
    }
    if ((buff & rst::Buffers::Depth) == rst::Buffers::Depth)
    {
        ...
        for(int i = 0; i < height * width; i++) {
            for(int j = 0; j < N * N; j++) {
                // depth: initial the infinity depth.
